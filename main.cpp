...
int main(int argc, char *argv[])
{
    ...
    unsigned int number_of_realizations = 10;
    for (int realization_counter=0; realization_counter<number_of_realizations; realization_counter++)
    {
        ModelParameters mp;
        ...
        mp.SetOutputfolder("Case " + QString::number(realization_counter));

        if (mp.GetMode() == Mode::heterogeneous)
        {
            PropertyGenerator *P = new PropertyGenerator(mp.GridSize("z"), realization_counter * 505);
            P->Generate(&mp);
